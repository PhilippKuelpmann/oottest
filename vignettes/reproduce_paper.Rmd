...
# Set up
The data as well as predictions for each theory mentioned in @kulpmann2020comparing are included as additional data to the package "oottest". Thus, you only need to install the package oottest. Furthermore, we use the package xtables to convert the R tables to latex tables which we use in the paper. 
...
The graph itself is created in latex using Tikz (TODO: Cite) which uses the data from R as inputs.
...
Example for the hawk-dove games:
```{r, echo = FALSE, results = 'asis'}
knitr::kable(chi_sq_hdg, caption="Chi-squared test for the hawk-dove games")
```

Now, to create the latex tables as seen in the paper:
```{r}
